...
Future<void> syncDevices({bool silent = false}) async {
  ...
  for (int i = 0; i < _devices.length; i++) {
    ...
    if (status != null) {
      ...
      // Auto-control for water pump with THRESHOLD CONFIRMATION DELAY
      if (device.type == DeviceType.waterPump && _appMode == AppMode.remote) {
        ...
        bool shouldTurnOn = waterLevel <= _pumpMinThreshold && !device.isOn;
        bool shouldTurnOff = waterLevel >= _pumpMaxThreshold && device.isOn;
        bool emergencyStop = waterLevel >= emergencyStopLevel && device.isOn;
        
        if (shouldTurnOn || shouldTurnOff || emergencyStop) {
          // 2-SECOND CONFIRMATION DELAY
          if (kDebugMode) {
            print('⏱️  Threshold hit detected, waiting 2 seconds for confirmation...');
          }
          ...
          // Re-check the water level after delay
          ...
          if (confirmStatus != null && confirmStatus['waterLevel'] != null) {
            ...
            if (kDebugMode) {
              print('✓ Confirmed water level: $confirmedLevel%');
            }
            
            // Re-evaluate with confirmed level
            if (confirmedLevel <= _pumpMinThreshold && !device.isOn) {
              for (int i = 5; i > 0; i--) {
            ...
            else if (confirmedLevel >= _pumpMaxThreshold && device.isOn) {
            ...
            else if (confirmedLevel >= emergencyStopLevel && device.isOn) {
              ...
              _addLog(
                ...
                deviceName: device.name,
                type: LogType.warning,
                action: 'EMERGENCY STOP (confirmed)',
                details: 'Water level reached $confirmedLevel%',
              );
 ...
 Future<bool> toggleDevice(String id) async {
    ...
    // SPECIAL HANDLING FOR WATER PUMP
    if (device.type == DeviceType.waterPump && newState == true) {
      _setExecutionStatus('⏱️ Verifying command (5 seconds)...');
      notifyListeners();
      
      // 5-second verification delay as per requirement
      for (int i = 5; i > 0; i--) {
